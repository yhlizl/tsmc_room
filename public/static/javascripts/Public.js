...
function WebSocketConnect(userInfo, toUserInfo = null) {
    if ("WebSocket" in window) {
        ...
        ws.onopen = function() {
            ws.send(send_data);
            //console.log("send_data 發送數據", send_data)
        };
        ...
        // 		console.log("to_user_send_data 發送數據", to_user_send_data)
        // 	};
        // }
        ...
        ws.onmessage = function(evt) {
            ...
            switch (received_msg.status) {
                ...
                case -1:
                    ws.close() // 主動close掉
                    console.log("client 連接已關閉...");
                    break;
                case 4:
                    $('.popover-title').html('在線用戶 ' + received_msg.data.count + ' 人')
        ...
        ws.onclose = function() {
            ...
            chat_info.html(chat_info.html() +
                ...
                "與服務器連接斷開，請刷新頁面重試" +
                '</span></li>');
            // let c = ws.close() // 主動close掉
            console.log("serve 連接已關閉... " + _time());
            // console.log(c);
        };

        ws.onerror = function(evt) {
            ws.close()
            console.log("觸發 onerror", evt)
        }
...
$(document).ready(function() {
    ...
    $('#userinfo_sub').click(function(event) {
        var userName = $('.rooms .user_name input').val(); // 用戶暱稱
        var userPortrait = $('.rooms .user_portrait img').attr('portrait_id'); // 用戶頭像id
        if (userName == '') { // 如果不填用戶暱稱，就是以前的暱稱
        ...
        $('.userinfo a b').text(userName); // 修改標題欄的用戶暱稱
        $('.rooms .user_name input').val(''); // 暱稱輸入框清空
        $('.rooms .user_name input').attr('placeholder', userName); // 暱稱輸入框默認顯示用戶暱稱
        $('.topnavlist .popover').not($(this).next('.popover')).removeClass('show'); // 關掉用戶面板
        $('.clapboard').addClass('hidden'); // 關掉模糊背景
    });
    ...
    $('.theme img').click(function(event) {
        ...
        $('body').css('background-image', 'url(images/theme/' + theme_id + '_bg.jpg)'); // 設置背景
    });
    ...
    $('.imgFileBtn').change(function(event) {
        ...
        $.ajax({
            ...
            sends_message($('.room').attr('data-username'), $('.room').attr('data-avatar_id'), str); // sends_message(暱稱,頭像id,聊天內容);
    ...
    $('#subxx').click(function(event) {
        //var str = $('.text input').val(); // 獲取聊天內容
        var str = $("#emojionearea2")[0].emojioneArea.getText() // 獲取聊天內容
        str = str.replace(/\</g, '&lt;');
        str = str.replace(/\>/g, '&gt;');
        str = str.replace(/\n/g, '<br/>');
